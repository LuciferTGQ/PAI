...
不同数据规模的一个 epoch 包含完全不同的数据量，不能仅按轮数横向判断训练是否充分。M100、M1000和M10000每轮分别暴露100,000、1,000,000和10,000,000个状态转移。各模型均按预设训练预算完整训练，并从保存的候选 checkpoint 中依据统一验证的单步与多步预测表现选择参数。部分模型在训练后期仍能降低训练误差，但验证误差已经趋于饱和甚至回升，因此最终采用中间 checkpoint，而不是机械使用最后一轮参数。

\input{generated/table_training_sufficiency}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{fig9_training_sufficiency.pdf}
  \caption{\textbf{正式训练历史与中间 checkpoint。} M1000 Transformer-2L 完成50轮预设训练；训练 MSE 持续下降，而验证 MSE 在中后期趋于饱和并出现回升。虚线标出统一验证协议最终采用的架构内第20轮 checkpoint；该位置由单步与多步 NRMSE 综合确定，并非仅按验证 MSE 的最低点选择。}
