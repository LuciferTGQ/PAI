...
- 核心结论：单步误差接近的模型在更长预测步长下可能呈现不同的递归预测偏差。
...
## 图 9：代表性正式训练过程与候选 checkpoint 评价

- 核心结论：正式50轮训练已完成，但训练误差持续下降时验证误差并未继续同步改善，因此应比较保存的中间 checkpoint。
- 主要证据：M1000 Transformer-2L 正式训练 CSV 中的训练/验证 MSE，以及11个实际候选 checkpoint 的单步和 H5/H10/H20 平均 NRMSE。
- 审阅风险：候选 checkpoint 之间的连线仅用于辅助观察，不得被理解为插值或连续评价；最终轮次仍由统一验证协议确定。
