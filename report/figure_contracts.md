...
## 图 9：正式训练历史与中间 checkpoint

- 核心结论：正式50轮训练已完成，但训练误差持续下降时验证误差并未继续同步改善，因此应比较保存的中间 checkpoint。
- 主要证据：M1000 Transformer-2L 正式训练 CSV 中的训练 MSE、验证 MSE 与架构内选定轮次。
- 审阅风险：最终 checkpoint 由统一验证的单步与多步 NRMSE 选择，图中的验证 MSE 仅用于解释训练过程，不能被误读为唯一选择指标。
